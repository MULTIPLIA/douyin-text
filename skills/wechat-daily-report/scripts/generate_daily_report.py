...
def build_session() -> requests.Session:
    ...
    offershow_token = os.getenv("OFFERSHOW_ACCESS_TOKEN", "").strip()
    if offershow_token:
...
def build_wechat_report(
    ...
    if not offers:
        if latest_public_offer_date and latest_public_offer_date < target_offer_date:
            jobs_lines.append(
...
def fetch_offershow_data(
    ...
    *,
    ...
    for page in range(1, max_pages + 1):
        ...
        page_plans = plan_payload["data"]["plans"]
        if not page_plans:
            break
